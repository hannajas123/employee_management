...
async function loadForm(formId) {
  const res = await axios.get(`${baseURL}forms/${formId}/`, { headers: getAuthHeader() });
  const form = res.data;
  const container = document.getElementById("employeeForm");
  container.innerHTML = "";
  form.fields.forEach(f => {
    container.innerHTML += `<label>${f.label}</label><input type="${f.field_type}" id="${f.label}" /><br>`;
  });
}

async function submitEmployee(formId) {
  const res = await axios.get(`${baseURL}forms/${formId}/`, { headers: getAuthHeader() });
  const form = res.data;
  let data = {};
  form.fields.forEach(f => {
  ...
  await axios.post(`${baseURL}employees/`, { data }, { headers: getAuthHeader() });
  alert("Employee saved!");
}

async function loadEmployees() {
  const res = await axios.get(`${baseURL}employees/`, { headers: getAuthHeader() });
  const list = document.getElementById("employeeList");
  list.innerHTML = "";
  res.data.forEach(emp => {
    list.innerHTML += `<li>${JSON.stringify(emp.data)} 
      <button onclick="deleteEmployee(${emp.id})">Delete</button></li>`;
  });
}

async function deleteEmployee(id) {
  await axios.delete(`${baseURL}employees/${id}/`, { headers: getAuthHeader() });
  loadEmployees();
}
